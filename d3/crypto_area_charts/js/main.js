...
// Add line path element to the graph
g.append("path")
    .attr("class", "line");
    // .attr("fill", "none")
...
function update(){
    ...
    // get min y-value of data selection
    var dataYMin = d3.min(dateCoinFilteredData, function(d) { return d[varValue]; })
    ...
    // Define path generator for our area
    var area = d3.area()
	.x(function(d){ return xScale(d.date); })
	.y0(yScale(dataYMin))
	.y1(function(d){ return yScale(d[varValue]); });
    ...
    // Update area path (using smooth transition)
    g.select(".area")
	.transition(trans)
	.attr("d", area(dateCoinFilteredData));
